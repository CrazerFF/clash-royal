(() => {
  ...
  var ce = (e, r, t) =>
    r in e ? he(e, r, { enumerable: !0, configurable: !0, writable: !0, value: t }) : (e[r] = t);
  var o = (e, r, t) => (ce(e, typeof r != 'symbol' ? r + '' : r, t), t);
  var g = /* @__PURE__ */ ((e) => (
    (e.active = 'pixi-active'),
    (e.inactive = 'pixi-inactive'),
    (e.stateUpdate = 'pixi-state-update'),
    (e.pulse = 'pixi-pulse'),
    (e.panelShown = 'devtool:panelShown'),
    (e.panelHidden = 'devtool:panelHidden'),
    (e.pageReload = 'devtool:pageReload'),
    (e.overlayStateUpdate = 'pixi-overlay-state-update'),
    e
  ))(g || {});
  class v {
    constructor(r) {
  ...
  class ue extends v {
    constructor() {
      ...
      o(this, '_previousData', []);
    }
    init() {
      (this._textures.clear(), this._gpuTextureSize.clear());
    }
    reset() {
      (this._textures.clear(), this._gpuTextureSize.clear(), (this._previousData = []));
    }
    update() {}
    get() {
      const t = this._devtool.renderer.texture.managedTextures,
        s = this.getWebTextures(),
        i = [];
      if (
        (t.forEach((n) => {
          if (n && n.resource) {
        ...
        !('_glTextures' in this._devtool.renderer.texture))
      ) {
        const n = new Set(i.map((a) => a.uid));
        (this._previousData.forEach((a) => {
          n.has(a.uid) || i.push({ ...a, isLoaded: !1 });
        }),
          (this._previousData = i.slice()));
      }
      return i;
    }
    getWebTextures() {
      ...
      const i = t.texture;
      return (
        '_glTextures' in i || '_gpuSources' in i
          ? (s = '_glTextures' in i ? i._glTextures : i._gpuSources)
          : (s = i.managedTextures.reduce((a, l) => {
              if (!l) return a;
              const d = l._gpuData[this._devtool.renderer.uid];
              return (d && (a[l.uid] = 'gpuTexture' in d ? d.gpuTexture : d), a);
            }, {})),
        s
      );
    }
    getTextureSource(t) {
      return t.resource instanceof ImageBitmap ||
        ...
        t.resource instanceof HTMLVideoElement
        ? this._imageBitmapToString(t.resource)
        : t.resource instanceof HTMLCanvasElement
          ? this._canvasToString(t.resource)
          : t.uploadMethodId === 'image' || t.uploadMethodId === 'video'
            ? t.resizeCanvas
              ? this._canvasToString(t.resource)
              : this._imageBitmapToString(t.resource)
            : null;
    ...
    _imageBitmapToString(t) {
      return (
        ...
        this._canvas.getContext('2d').drawImage(t, 0, 0),
        this._canvasToString(this._canvas)
      );
    }
    _canvasToString(t) {
    ...
    getMemorySize(t, s) {
      return s
        ? Array.isArray(t.resource) && t.resource[0] instanceof Uint8Array
          ? t.resource.reduce((i, n) => i + n.byteLength, 0)
          : typeof s.format == 'string' && $[s.format]
            ? s.width * s.height * $[s.format]
            : typeof s.format == 'number' && X[s.format]
              ? s.width * s.height * X[s.format]
              : null
        : null;
    }
  }
    ...
    _ = {
      ...
      handleByNamedList(e, r, t = 0) {
        return this.handle(
          e,
          (s) => {
            r.findIndex((n) => n.name === s.name) >= 0 ||
              (r.push({ name: s.name, value: s.ref }),
              r.sort((n, a) => E(a.value, t) - E(n.value, t)));
          },
          (s) => {
      ...
      handleByList(e, r, t = 0) {
        return this.handle(
          e,
          (s) => {
            r.includes(s.ref) || (r.push(s.ref), r.sort((i, n) => E(n, t) - E(i, t)));
          },
          (s) => {
  ...
  function ge(e, r) {
    return r
      ? r.BitmapText && e instanceof r.BitmapText
      : ('renderPipeId' in e && e.renderPipeId === 'BitmapText') || '_activePagesMeshData' in e;
  }
  function fe(e, r) {
    return r
      ? r.HTMLText && e instanceof r.HTMLText
      : ('renderPipeId' in e && e.renderPipeId === 'htmlText') ||
          ('_foreignObject' in e && '_svgRoot' in e);
  }
  function te(e, r) {
    return r
      ? r.Text && e instanceof r.Text
      : ('renderPipeId' in e && e.renderPipeId === 'text') ||
          ('updateText' in e && 'drawLetterSpacing' in e && '_render' in e);
  }
  function _e(e, r) {
    return r
      ? r.Mesh && e instanceof r.Mesh
      : ('renderPipeId' in e && e.renderPipeId === 'mesh') ||
          ('_geometry' in e && 'drawMode' in e && 'vertexData' in e && 'batchUvs' in e);
  }
  function ye(e, r) {
    return r
      ? r.Graphics && e instanceof r.Graphics
      : ('renderPipeId' in e && e.renderPipeId === 'graphics') ||
          ('drawRect' in e && 'drawPolygon' in e);
  }
  function re(e, r) {
    return r
      ? r.AnimatedSprite && e instanceof r.AnimatedSprite
      : 'gotoAndPlay' in e && 'stop' in e && 'play' in e && '_isConnectedToTicker' in e;
  }
  function se(e, r) {
    return r
      ? r.NineSliceSprite && e instanceof r.NineSliceSprite
      : ('renderPipeId' in e && e.renderPipeId === 'nineSliceSprite') ||
          ('_leftWidth' in e &&
            '_rightWidth' in e &&
            '_topHeight' in e &&
            '_bottomHeight' in e &&
  ...
  function ie(e, r) {
    return r
      ? r.TilingSprite && e instanceof r.TilingSprite
      : ('renderPipeId' in e && e.renderPipeId === 'tilingSprite') ||
          ('tileTransform' in e && 'uvRespectAnchor' in e && 'uvMatrix' in e);
  }
  function me(e, r) {
    return r
      ? r.Sprite && e instanceof r.Sprite
      : ('renderPipeId' in e && e.renderPipeId === 'sprite') ||
          ('vertexTrimmedData' in e && 'indices' in e);
  }
  function xe(e, r) {
    return r
      ? r.Container && e instanceof r.Container
      : ('includeInBuild' in e && 'measurable' in e && '_didLocalTransformChangeId' in e) ||
          ('_maskRefCount' in e && '_render' in e && '_tempDisplayObjectParent' in e);
  }
  function W(e, r) {
    return r
      ? r.ParticleContainer && e instanceof r.ParticleContainer
      : ('renderPipeId' in e && e.renderPipeId === 'particle') || 'particleChildren' in e;
  }
  function G(e) {
    const r = c.pixi;
    return ge(e, r)
  ...
  function ne(e, r) {
    ...
    if (i !== !1 && (t(e, e.parent), !(e.children.length === 0 || i === 'children')))
      for (let n = 0; n < e.children.length; n++) ne(e.children[n], r);
  }
  function P(e, r) {
    const t = c;
    if (!r.has(e)) {
      const s = t.textures.getTextureSource(e),
        i = t.textures.getWebTextures(),
        n = t.textures.getMemorySize(e, i[e.uid]);
      s &&
        r.set(e, {
          blob: s,
          name: e.label,
  ...
  function Te(e, r) {
    const t = c.renderer.name === 'webgl' ? 'webgl' : 'webgpu',
      s = (i, n) => {
        const a = t === 'webgl' ? 'glProgram' : 'gpuProgram';
        let l;
        if ((i.blurXFilter ? (l = i.blurXFilter[a]) : (l = i[a]), !l)) return '';
        const d = l[n];
    ...
    return {
      ...
      filter: e.filterEffect?.filters?.map((i) => ({
        ...
        padding: i.padding,
        resolution: typeof i.resolution == 'number' ? i.resolution : 1,
  ...
  function Ee(e, r) {
    const t = e?.mesh || e,
      s = c.renderer.name === 'webgl' ? 'webgl' : 'webgpu';
    let i = t.shader;
    const n = {
      fragment: null,
      vertex: null,
    ...
    return (
      i
        ? ((n.fragment = I(i, 'fragment', s)), (n.vertex = I(i, 'vertex', s)))
        : ((i = c.renderer.renderPipes.mesh._adaptor._shader),
          (n.fragment = I(i, 'fragment', s)),
          (n.vertex = I(i, 'vertex', s))),
      {
        type: 'mesh',
        action: 'draw',
        renderable: {
  ...
  function Ce(e, r, t, s, i) {
    const n = new Uint8Array(s * i * 4),
      a = r.renderTarget.getRenderTarget(r.renderTarget.renderTarget),
      l = r.renderTarget.getGpuRenderTarget(a);
    (e.bindFramebuffer(e.FRAMEBUFFER, l.resolveTargetFramebuffer),
      e.readPixels(
        0,
        0,
    ...
    const p = d.getContext('2d'),
      h = new ImageData(new Uint8ClampedArray(n), s, i);
  ...
  let Me = class {
    ...
    update() {
      ...
      if (r >= this.prevTime + 1e3) {
        ((this.fps = (this.frames * 1e3) / (r - this.prevTime)),
          (this.prevTime = r),
          (this.frames = 0));
        const t = performance.memory;
        ((this.memory = t.usedJSHeapSize / 1048576),
          (this.maxMemory = t.jsHeapSizeLimit / 1048576));
      }
    }
    reset() {
      ((this.frames = 0),
  ...
  class Ae extends v {
    ...
    reset() {
      if (this._devtool.majorVersion !== '8') return;
      const t = this._devtool.renderer;
      if (this._batcherBuildStartFn)
        if (((t.renderPipes.batch.buildStart = this._batcherBuildStartFn), t.type === 2)) {
          const i = t.encoder;
          ((i.beginRenderPass = this._originalBeginRenderPass),
            (i.restoreRenderPass = this._originalRestoreRenderPass));
        } else {
          const s = t;
    ...
    init() {
      ...
      const t = this._devtool.renderer;
      if (
        ((this._batcherBuildStartFn = t.renderPipes.batch.buildStart),
        (t.renderPipes.batch.buildStart = (...n) => {
        ...
        const n = t,
          ...
          l = n.encoder;
        ((this._originalBeginRenderPass = l.beginRenderPass),
          (l.beginRenderPass = (...d) => {
            const p = this._originalBeginRenderPass.apply(l, d);
            return (a('draw'), a('drawIndexed'), p);
          }),
          (this._originalRestoreRenderPass = l.restoreRenderPass),
          (l.restoreRenderPass = (...d) => {
      ...
      s.geometry.draw = (...n) => {
        ...
        if ((this.stats.drawCalls++, !this._capturing || !this._withScreenshot)) return a;
        const l = i.drawingBufferWidth,
          d = i.drawingBufferHeight;
    ...
    captureCanvasData() {
      ...
      let n = 'default',
        ...
        d = '';
      if (t.type !== 2) {
        const h = s.gl.getContextAttributes();
        ((n = h.powerPreference),
          (a = h.failIfMajorPerformanceCaveat?.toString()),
          (l = h.preserveDrawingBuffer?.toString() ?? ''),
          (d = h.premultipliedAlpha?.toString() ?? ''));
      }
      const p = {
      ...
      return '_glTextures' in t.texture || '_gpuSources' in t.texture
        ? {
            ...p,
        ...
        : {
            ...
            gcActive: t.gc.enabled.toString(),
            // @ts-expect-error - newer pixi version
            gcMaxUnusedTime: t.gc.maxUnusedTime.toString(),
            // @ts-expect-error - newer pixi version
            gcFrequency: t.gc._frequency.toString(),
          };
    }
    ...
    capture(t) {
      this._textureCache.clear();
      const s = this._devtool.renderer,
        i = s.lastObjectRendered,
        n = i.renderGroup.instructionSet,
        a = {
          instructions: [],
          drawCalls: 0,
          totals: {
        ...
        l = this._capture(s, t),
        d = this._getSceneData(i),
        p = this._getInstructionsData(n, l, this._drawOrder);
    ...
    _capture(t, s) {
      ...
      return (
        Object.keys(n).forEach((a) => {
          const l = n[a];
          l.execute &&
            (this._pipeExecuteFn.set(a, l.execute),
            (l.execute = (...d) => (
              (this._currentPipe = a),
              this._drawOrder.push({ pipe: a, drawCalls: 0 }),
              this._pipeExecuteFn.get(a).apply(l, d)
            )));
        }),
        t.render(t.lastObjectRendered),
        this._postCapture(t),
        this._canvasTextures
      );
    }
      ...
      return (
        S({
          ...
          loop(i) {
            ...
            (i.effects && (s.filters += Array.isArray(i.effects) ? i.effects.length : 1),
              i.mask && s.masks++);
          },
        }),
        s
    ...
    _getInstructionsData(t, s, i, n = 0, a = 0) {
      ...
      for (let d = 0; d < t.instructionSize; d++) {
        ...
        switch (p.renderPipeId) {
          ...
          case 'renderGroup':
            {
              const T = this._getInstructionsData(p.instructionSet, s, i, n, a + 1);
              l.push(...T);
            }
            break;
          default:
        ...
        if (!h) continue;
        const y = i[a];
        ((h.drawCalls = y.drawCalls),
          (n += h.drawCalls),
          (h.drawTextures = s.slice(n - h.drawCalls, n)),
          a++,
          l.push(h));
      }
      return (
  ...
  function Ne(e, r = !0) {
    if (!e && r) throw new Error('[PixiDevtools] Extension not found.');
    return e;
  }
  function f(e, r) {
  ...
  const D = class D extends v {
    ...
    init() {
      this._extensions = f(D.extensions, 'track');
      const t = [];
      for (const i of this._extensions) t.push(...i.getKeys());
      const s = t.filter((i, n) => t.indexOf(i) !== n);
      s.length > 0 &&
        console.warn(`[PixiJS Devtools] Stats: Duplicate keys found: ${s.join(', ')}`);
    }
    reset() {
      this.stats = {};
    }
    ...
    loop(t) {
      for (const s of this._extensions) s.track(t, this.stats);
    }
    postupdate() {
  ...
  const w = class w extends v {
    constructor(t) {
      ...
      const s = (n) => {
          n.altKey &&
            window.postMessage(
              { method: g.overlayStateUpdate, data: { overlayPickerEnabled: !0 } },
              '*'
            );
        },
        i = (n) => {
          n.altKey ||
            window.postMessage(
              { method: g.overlayStateUpdate, data: { overlayPickerEnabled: !1 } },
              '*'
            );
        };
    ...
    init() {
      ...
      ((this._highlightEnabled = !1),
        (this._pickerEnabled = !1),
        t !== this._canvas &&
          ((this._canvas = t),
          this._buildOverlay(),
          (this._selectedStylesExt = L(w.extensions, 'getSelectedStyle')),
          (this._hoverStylesExt = L(w.extensions, 'getHoverStyle')),
    ...
    enablePicker(t) {
      ((this._pickerEnabled = this._keydown ? !0 : t),
        this._pickerEnabled ? this.activatePick() : this.disablePick());
    }
    activatePick() {
    ...
    _updateOverlay() {
      const t = this._devtool.canvas,
        s = this._devtool.renderer,
        n = this._devtool.majorVersion === '8' ? 1 : s.resolution;
      Object.assign(this._overlay.style, {
        width: `${s.width / n}px`,
    ...
    _hitTest(t) {
      ...
      S({
        ...
        loop: (l) => {
          (i.set(l, l.eventMode === void 0 ? l.interactive : l.eventMode),
            (l.interactive = !l.__devtoolLocked));
        },
      });
      const n = { x: 0, y: 0 };
  ...
  const u = class u extends v {
    ...
    init() {
      ((F = /* @__PURE__ */ new WeakMap()),
        (q = 0),
        (this._metadataExtensions = f(u.extensions, 'updateNodeMetadata')),
        (this._onButtonPressExtensions = f(u.extensions, 'onButtonPress')),
        (this._onContextMenuExtensions = f(u.extensions, 'onContextButtonPress')),
        (this._onRenameExtensions = f(u.extensions, 'onRename')),
        (this._onDeletedExtensions = f(u.extensions, 'onDeleted')),
        (this._onSwapExtensions = f(u.extensions, 'onSwap')),
        (this._onSelectedExtensions = f(u.extensions, 'onSelected')),
        (this._treePanelButtons = f(u.extensions, 'panelButtons')),
    ...
    getSceneGraph() {
      return !this._devtool.stage ||
        this._devtool.renderer.lastObjectRendered !== this._devtool.stage
        ? null
        : (this.clear(),
          S({
            container: this._devtool.stage,
          ...
          this.selectedNode &&
            !this._sceneGraph.has(this.selectedNode) &&
            ((this.selectedNode = null), (window.$pixi = null)),
          {
    ...
    updateLoop(t, s = !0) {
      const i = this._devtool.stage,
        n = G(t),
        { suffix: a, name: l } = this._getName(t),
        d = {
          id: this._getId(t),
          name: l,
          children: [],
      ...
      if (
        (this._metadataExtensions.forEach((p) => {
          p.updateNodeMetadata(t, d.metadata);
        }),
        this._idMap.set(d.id, t),
        t === i)
      ) {
    ...
    nodeButtonPress(t, s, i) {
      ...
      n &&
        (s === 'locked' && (n.__devtoolLocked = i),
        this._onButtonPressExtensions.forEach((a) => {
    ...
    nodeContextMenu(t, s) {
      ...
      i &&
        this._onContextMenuExtensions.forEach((n) => {
    ...
    setSelected(t) {
      ...
      ((this.selectedNode = this._idMap.get(t) ?? null),
        (window.$pixi = this.selectedNode),
        this._onSelectedExtensions.forEach((s) => {
    ...
    moveNode(t, s, i) {
      ...
      !n ||
        ...
        a.addChildAt(n, i),
        this._onSwapExtensions.forEach((l) => {
          l.onSwap(n, i);
        }));
    }
    renameNode(t, s) {
      ...
      i &&
        (i.__devtoolLocked ||
          (this._devtool.majorVersion === '8' ? (i.label = s) : (i.name = s),
          this._onRenameExtensions.forEach((n) => {
            n.onRename(i, s);
          })));
    }
    deleteNode(t) {
      ...
      s &&
        (s.__devtoolLocked ||
          ...
          this._onDeletedExtensions.forEach((i) => {
            i.onDeleted(s);
          })));
    }
    logSelected() {
    ...
    _getName(t) {
      ...
      const i = this._devtool.stage,
        n = this._devtool.majorVersion === '8' ? t.label : t.name,
        l = n === s || !n ? s : n;
    ...
    _getId(t) {
      const s = F.get(t);
      if (s) return s;
      const i = `${q++}_${(Math.random() + 1).toString(36).substring(2)}`;
      return (F.set(t, i), i);
    }
  };
  o(u, 'extensions', []);
  ...
  class ke extends v {
    ...
    constructor(t) {
      ...
      o(this, 'properties');
      ((this.stats = new R(t)),
        (this.overlay = new N(t)),
        (this.tree = new k(t)),
        (this.properties = new H(t)));
    }
    init() {
  ...
  const He = {
    ...
    Oe = {
      ...
      getProperties(e) {
        return this.properties().reduce((t, s) => {
          ...
          let n = e[i];
          return (
            n != null && (i === 'gotoAndPlay' || i === 'stop' || i === 'play') && (n = !0),
            (n == null && s.allowUndefined !== !0) ||
              t.push({
                ...s,
                value: n,
              }),
    ...
    m = (e) => [e.x, e.y],
    C = (e) => [e.x, e.y, e.width, e.height],
    Ge = (e) => [e.a, e.b, e.c, e.d, e.tx, e.ty],
    x = (e, r, t) => e[r].set(t[0], t[1]),
    M = (e, r, t) => {
      ((e[r].x = t[0]), (e[r].y = t[1]), (e[r].width = t[2]), (e[r].height = t[3]));
    },
    Le = (e, r, t) => e[r].fromArray(t),
    ae = [
...
{{import 'pixi.js/advanced-blend-modes';}}`,
        ...
        return this.properties().reduce((t, s) => {
          ...
          return (
            i === 'type'
              ...
              : i === 'blendMode' && c.majorVersion === '7'
                ? (n = Object.keys(j).find((a) => j[a] === n))
                : n != null && J[i] && (n = J[i](n)),
            (n == null && s.allowUndefined !== !0) ||
              t.push({
                ...s,
  ...
  class et {
      ...
      for (const t of r) {
        ...
        if (window.frames)
          for (let s = 0; s < window.frames.length; s += 1)
            try {
              if (window.frames[s][t]) return window.frames[s][t];
            } catch {}
      }
    }
    get devtools() {
      if (this._devtools) return this._devtools;
      if (((this._devtools = this.searchIFrames(['__PIXI_DEVTOOLS__'])), !this._devtools)) return;
      const r = this._devtools.extensions || [];
      return (
        (this._devtools.extensions = r),
        this._devtools.extensions.forEach((t) => {
          _.add(t);
        }),
        this._devtools
      );
    ...
    get app() {
      return this._app
        ...
        : ((this._app = this.devtools?.app),
          this._app ? this._app : ((this._app = this.searchIFrames(['__PIXI_APP__'])), this._app));
    }
    /**
    ...
    get stage() {
      return this._stage
        ...
        : ((this._stage = this.devtools?.stage),
          this._stage
            ? this._stage
            : ((this._stage = this.searchIFrames(['__PIXI_STAGE__'])),
              this._stage
                ? this._stage
                : ((this._stage = this.app?.stage),
                  this._stage
                    ? this._stage
                    : ((this._stage = this.renderer?.lastObjectRendered), this._stage))));
    }
    /**
        ...
        : ((this._renderer = this.devtools?.renderer),
          this._renderer
            ...
            : ((this._renderer = this.searchIFrames(['__PIXI_RENDERER__'])),
              this._renderer
                ? this._renderer
                : ((this._renderer = this.app?.renderer), this._renderer)));
    }
    /**
    ...
    get pixi() {
      return this._pixi
        ...
        : ((this._pixi = this.devtools?.pixi),
          this._pixi
            ? this._pixi
            : ((this._pixi = this.searchIFrames(['PIXI', '__PIXI__'])), this._pixi));
    }
    /**
    ...
    get version() {
      return this._version
        ? this._version
        : ((this._version = this._devtools?.version ?? this.pixi?.VERSION ?? ''), this._version);
    }
    /**
    ...
    inject() {
      const r = this;
      this.renderer &&
        !this.renderer.__devtoolInjected &&
        ((this.renderer.__devtoolInjected = !0),
    ...
    reset() {
      (this.renderer && this._originalRenderFn && (this.renderer.render = this._originalRenderFn),
        this.renderer && (this.renderer.__devtoolInjected = !1),
        ...
        (this._initialized = !1),
        window.postMessage({ method: g.pulse, data: {} }, '*'));
    }
    update() {
      (this._initialized || this.init(),
        this.preupdate(),
        this._update(),
        this._updateThrottle.shouldExecute(this.settings.throttle) &&
          (this.updatedThrottled(),
          this.renderer.lastObjectRendered === this.stage &&
            S({
              ...
              test: (r) =>
                r.__devtoolIgnore ? !1 : r.__devtoolIgnoreChildren || W(r) ? 'children' : !0,
            }),
          this.postupdate()));
    }
    init() {
  ...
  function it(e) {
    try {
      ...
      r === g.active
        ? (U(), e())
        : window.postMessage({ method: r, data: JSON.stringify({}) }, '*');
    } catch {
      U();
    }
  }
